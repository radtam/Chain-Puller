...
#define STEPS_PER_REV_DEF 100   // this value needs to correspond to the  switch setting on the DM1182 myStepperDrive drive 
#define MAX_SPEED_DEF   20   // revolutions per second
#define ACCELERATION_DEF  5000  // steps per sec
...
//                    "0123456789012345"
const char display0[] PROGMEM = "SK11686"; // This what is displayed when the arduino turns on
...
const char display42[] PROGMEM = "Servo P1 Angle  ";  // Neutral angle  
...
struct CycleTester {
  ...
  int servo_p1_angle; // neutral angle
...
uint32_t sTime = 0;   // used for keeping track of relative time
...
void setup() {
  ...
  init_myCycleTest();           // initialize the Cycle Test data v
...
void init_myCycleTest(void) {
  EEPROM.get(EE_ADDR_CYCLE_DATA,myCycleTest); // these variables may be in EPROM, may need to manually set on board interface

  if (myCycleTest.chksum != EE_CHKSUM_VALID) {
    myCycleTest.cycle_count = 0;
    myCycleTest.num_cycles = 1000; // changed default to 1000 for use with servo system Dirk Rettig 23MAY2022
    myCycleTest.dwell_time = 15;
    myCycleTest.total_test_time = 0;
    myCycleTest.move_time = 0;
    myCycleTest.duty_cycle = 50;
    myCycleTest.move_timeout = 10;
    myCycleTest.index = 0;
...
void updateEditVariables(void) {
  switch (menuLCD[myLCD.top_menu_index][myLCD.sub_menu_index]) {
    ...
    case INDEX_INV_DOWN:
      ...
      break;
    case INDEX_SERVO_P1_ANGLE:
      myEdit.max_Value = 360;
      myEdit.min_Value = 0;
      myEdit.stepSize = 1;
      myEdit.xMultipler = 5;
      myEdit.cur_Value =  myCycleTest.servo_p1_angle;
      break;
    case INDEX_SERVO_P2_ANGLE:
      myEdit.max_Value = 360;
      myEdit.min_Value = 0;
      myEdit.stepSize = 1;
      myEdit.xMultipler = 5;
      myEdit.cur_Value =  myCycleTest.servo_p2_angle;
      break;
    case INDEX_SERVO_P3_ANGLE:
      myEdit.max_Value = 360;
      myEdit.min_Value = 0;
      myEdit.stepSize = 1;
      myEdit.xMultipler = 5;
      myEdit.cur_Value =  myCycleTest.servo_p3_angle;
      break;
  }
...
void saveEdit(void) {
  ...
  switch (menuLCD[myLCD.top_menu_index][myLCD.sub_menu_index]) {
    ...
    case INDEX_INV_DOWN:
      ...
      break;  
    case INDEX_SERVO_P1_ANGLE:
      myCycleTest.servo_p1_angle = myEdit.cur_Value;
      saveCycleData();
      end_edit = true;
      break;
    case INDEX_SERVO_P2_ANGLE:
      myCycleTest.servo_p2_angle = myEdit.cur_Value;
      saveCycleData();
      end_edit = true;
      break;
    case INDEX_SERVO_P3_ANGLE:
      myCycleTest.servo_p3_angle = myEdit.cur_Value;
      saveCycleData();
      end_edit = true;
      break;      
  }
...
void updateDisp_SetupEdits(uint16_t theData) {
  ...
  switch (menuLCD[myLCD.top_menu_index][myLCD.sub_menu_index]) {
    ...
    case INDEX_INV_DOWN:
      ...
      break; 
    case INDEX_SERVO_P1_ANGLE:
      if (theData == INVALID_DATA) theData = myCycleTest.servo_p1_angle;
      lcd.setCursor(12,1); lcd.print("    ");
      lcd.setCursor(13,1); lcd.print(theData);
      myservo.write(theData);                                               
      break;
    case INDEX_SERVO_P2_ANGLE:
      if (theData == INVALID_DATA) theData = myCycleTest.servo_p2_angle;
      lcd.setCursor(12,1); lcd.print("    ");
      lcd.setCursor(13,1); lcd.print(theData);
      myservo.write(theData);                                               
      break;
    case INDEX_SERVO_P3_ANGLE:
      if (theData == INVALID_DATA) theData = myCycleTest.servo_p3_angle;
      lcd.setCursor(12,1); lcd.print("    ");
      lcd.setCursor(13,1); lcd.print(theData);
      myservo.write(theData);                                              
      break;
...
void monitorCycleTest(void) {
  ...
  //if (SAFETY_STOP == 1){ // Added by AP - set to 1 to be active
  // pauseCycleTest();
  //      }
  //else if (myLCD.row1_index == INDEX_CYCLE_ACTIVE) {
  Serial.println(myCycleTest.index);        //shade starts as the top and servo is in P1 (neutral)
